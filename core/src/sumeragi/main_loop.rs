...
#[instrument(skip(sumeragi, state_machine))]
#[allow(clippy::expect_used)]
/// Execute the main loop of [`SumeragiWithFault`]
pub fn run<F>(
    sumeragi: &SumeragiWithFault<F>,
    mut state_machine: State,
    ...
    F: FaultInjection,
{
    ...
    if state_machine.latest_block_height == 0
        || state_machine.latest_block_hash == Hash::zeroed().typed()
    {
        if let Some(genesis_network) = state_machine.genesis_network.take() {
            sumeragi_init_commit_genesis(sumeragi, &mut state_machine, genesis_network);
        } else {
    ...
    assert_eq!(
        state_machine.latest_block_hash,
        state_machine.wsv.latest_block_hash()
    );
    trace!(
        "I, {}, finished sumeragi init. My role in the next round is {:?}",
        sumeragi.peer_id.public_key,
        state_machine.current_topology.role(&sumeragi.peer_id),
    );
    ...
    loop {
        ...
        sumeragi.connect_peers(&state_machine.current_topology);

        {
            let state_machine = &mut state_machine;
        ...
        compare_view_change_index_and_block_height_to_old(
            ...
            state_machine.latest_block_height,
        ...
        handle_role_agnostic_messages(sumeragi, &mut state_machine, &mut maybe_incoming_message);

        if state_machine.current_topology.role(&sumeragi.peer_id) != Role::Leader {
            if !state_machine.transaction_cache.is_empty() && !has_sent_transactions {
                ...
                info!(
                    ...
                    leader_addr = %state_machine.current_topology.leader().address,
                    %tx_hash,
                ...
                if let Ok(MustUse(true)) = tx.check_signature_condition(&state_machine.wsv) {
                    sumeragi.broadcast_packet_to(
                        MessagePacket::new(
                            view_change_proof_chain.clone(),
                            crate::sumeragi::TransactionForwarded::new(tx).into(),
                        ),
                        [state_machine.current_topology.leader()].into_iter(),
                    );
        ...
        if state_machine.current_topology.role(&sumeragi.peer_id) == Role::ObservingPeer {
            if maybe_incoming_message.is_some() {
                ...
                match incoming_message {
                    ...
                    Message::BlockCommitted(block_committed) => {
                        ...
                        let block =
                            block.revalidate(&sumeragi.transaction_validator, &state_machine.wsv);
                        for event in Vec::<Event>::from(&block) {
                            trace!(?event);
                            sumeragi.events_sender.send(event).unwrap_or(0);
                        }

                        let network_topology = state_machine.current_topology.clone();
                        ...
                        if valid_signatures.len() >= network_topology.min_votes_for_commit()
                            ...
                            && state_machine.latest_block_hash == block.header().previous_block_hash
                        {
                            commit_block(sumeragi, block, &mut state_machine);
                        }
                    }
                    _ => {
        ...
        } else if state_machine.current_topology.role(&sumeragi.peer_id) == Role::Leader {
            if maybe_incoming_message.is_some() {
                ...
                match msg {
                    TransactionForwarded(transaction_forwarded) => {
                        ...
                        if transaction_maybe.is_ok() {
                            ...
                            match sumeragi.queue.push(transaction, &state_machine.wsv) {
            ...
            if voting_block_option.is_none() {
                if state_machine.transaction_cache.is_empty() {
                    instant_when_we_should_create_a_block = Instant::now() + sumeragi.block_time;
                    continue;
                }
                if Instant::now() > instant_when_we_should_create_a_block
                    || state_machine.transaction_cache.len() >= sumeragi.queue.txs_in_block
                {
                    let transactions: Vec<VersionedAcceptedTransaction> = state_machine
                        .transaction_cache
                        .iter()
                    ...
                    );
                    {
                        let block = {
                            ...
                            block.validate(&sumeragi.transaction_validator, &state_machine.wsv)
                        };
                        ...
                        if !state_machine.current_topology.is_consensus_required() {
                            sumeragi.broadcast_packet(
                                MessagePacket::new(
                                    view_change_proof_chain.clone(),
                            ...
                            commit_block(sumeragi, signed_block, &mut state_machine);
                            has_sent_transactions = false;
                            voting_block_option = None;
                        ...
                        sumeragi.broadcast_packet_to(
                            ...
                            state_machine.current_topology.peers_set_a().iter(),
                        );
        ...
        } else if state_machine.current_topology.role(&sumeragi.peer_id) == Role::ValidatingPeer {
            if let Some(incoming_message) = maybe_incoming_message.take() {
                match incoming_message {
                    Message::BlockCreated(block_created) => {
                        ...
                        {
                            error!(
                                role = ?state_machine.current_topology.role(&sumeragi.peer_id),
                                "Rejecting Block as it is not signed by leader.",
                            );
                            continue;
                        }

                        let hash = state_machine.latest_block_hash;
                        let block_height = state_machine.latest_block_height;
                        if let Err(e) = block.validation_check(
                            ...
                            sumeragi.broadcast_packet_to(
                                ...
                                [state_machine.current_topology.proxy_tail()].into_iter(),
                            );
                            info!(
                                peer_role = ?state_machine.current_topology.role(&sumeragi.peer_id),
                    ...
                    Message::BlockCommitted(block_committed) => {
                        ...
                        let valid_signatures =
                            state_machine.current_topology.filter_signatures_by_roles(
                                &[Role::ValidatingPeer, Role::Leader, Role::ProxyTail],
                                &verified_signatures,
                            );
                        if valid_signatures.len()
                            >= state_machine.current_topology.min_votes_for_commit()
                            && state_machine.latest_block_hash == block.header().previous_block_hash
                        {
                            commit_block(sumeragi, block, &mut state_machine);
                        }
        ...
        } else if state_machine.current_topology.role(&sumeragi.peer_id) == Role::ProxyTail {
            if maybe_incoming_message.is_some() {
            ...
            if voting_block_option.is_some() {
                ...
                let validating_peers = state_machine.current_topology.peers_set_a();
                ...
                if vote_count >= state_machine.current_topology.min_votes_for_commit() {
    ...
    F: FaultInjection,
{
    ...
    assert_eq!(state_machine.latest_block_height, 0);
    assert_eq!(state_machine.latest_block_hash, Hash::zeroed().typed());
    ...
    let block = PendingBlock::new(transactions, Vec::new())
        .chain_first_with_genesis_topology(state_machine.current_topology.clone());

    {
        info!(block_hash = %block.hash(), "Publishing genesis block.");

        let block = block.validate(&sumeragi.transaction_validator, &state_machine.wsv);
            ...
            .expect("Sign genesis block.");
        {
            ...
            commit_block(sumeragi, signed_block, state_machine);
        }
    }
}
...
{
    assert!(
        state_machine.current_topology.is_consensus_required(),
                ...
                Message::BlockCommitted(block_committed) => {
                    ...
                    if block_header.is_genesis() && block_header.genesis_topology.is_some() {
                        ...
                        state_machine.current_topology = block_header
                            ...
                            .take()
                            .expect("We just checked that it is some");
                        commit_block(sumeragi, block_committed.block, state_machine);
