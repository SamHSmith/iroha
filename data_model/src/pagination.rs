    ...
    fn from(pagination: Pagination) -> Self {
        match (pagination.start, pagination.limit) {
            (Some(start), Some(limit)) => {
                vec![(PAGINATION_START, start as usize), (PAGINATION_LIMIT, limit as usize)]
            }
            (Some(start), None) => vec![(PAGINATION_START, start as usize)],
            (None, Some(limit)) => vec![(PAGINATION_LIMIT, limit as usize)],
            (None, None) => Vec::new(),
        }
    }
}
